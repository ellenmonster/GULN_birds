...
table(site_covs$hab_type_200_comb) # For BITH there is only forest
...
yearlySiteCovs <- expand_dat %>%
  dplyr::select(location_name, loc_yr, yr_f, visit, researcher, first_yr, julian_prop_c, hrs_since_rise_sc, weather_wind_comb) %>% # Add 'yr_f' if doing yearmod; otherwise add something for trend
...
# These park units have multiple researchers -- BITH
# These park units have only one habitat type -- BITH
...
gdist_null_best <- gdistsamp(lambdaformula = ~yr_f, phiformula = ~1, pformula = ~1, data = umf_gds, keyfun = "halfnorm", output = "abund", mixture = "P", se = FALSE) # <<<<<<<< ENTER THE BEST MODEL KEYFUN AND MIXTURE. I can't simply read the list element corresponding to the best_distshape b/c causes problems when I try to update from this model
...
gdist_pfull <- update(gdist_null_best, pformula = ~researcher + first_yr + hrs_since_rise_sc + I(hrs_since_rise_sc^2) + understory_cover_sd_200, se = TRUE) # First p-model includes all likely p-covariates (anything with p> 0.5 from tmb trend model)
summary(gdist_pfull)

gdist_p1 <- update(gdist_null_best, pformula = ~researcher + first_yr, se = TRUE) # Then drop the less statistically significant ones (drop anything with p> 0.5)
summary(gdist_p1)

# # Have a model that drops julian_prop and hr_since_rise
# gdist_p2 <- update(gdist_null_best, pformula = ~researcher + first_yr + understory_cover_sd_200, se = TRUE) # Then drop the less statistically significant ones (drop anything with p> 0.5)
# summary(gdist_p2)

# # Also have a model with only researcher + first_yr, and another with only researcher
# gdist_p3 <- update(gdist_null_best, pformula = ~researcher + first_yr, se = TRUE) # Then drop the less statistically significant ones (drop anything with p> 0.5)
# summary(gdist_p3)

gdist_p4 <- update(gdist_null_best, pformula = ~researcher, se = TRUE) # Then drop the less statistically significant ones (drop anything with p> 0.5)
summary(gdist_p4)

## Model selection
(p_mods_table <- unmarked::fitList(fits=listN(gdist_null_best, gdist_pfull, gdist_p1, gdist_p4)) %>% unmarked::modSel(.))

gdist_p_best <- gdist_p4 # If a more complex covariate model is within approx. 2AIC of best-fit model, use it.
...
gdist_lambdafull <- update(gdist_phi_best, lambdaformula = ~yr_f + understory_cover_sd_200, se = TRUE) # First model includes all likely abundance covariates (anything with p> 0.5 from tmb trend model)
summary(gdist_lambdafull)

# gdist_lambda1 <- update(gdist_phi_best, lambdaformula = ~yr_f + julian_prop_c + hrs_since_rise_sc + understory_cover_sd_200, se = TRUE) # First model includes all likely abundance covariates (anything with p> 0.5 from tmb trend model)
# summary(gdist_lambda1)

# Also add a full(ish) model ----
# just add the lambdafull to the phifull
gdist_full <- update(gdist_phifull, lambdaformula = ~yr_f + understory_cover_sd_200, se = TRUE)
summary(gdist_full)

## Model selection so far...
unmarked::fitList(fits=listN(gdist_null_best, gdist_p_best, gdist_phifull, gdist_lambdafull, gdist_full)) %>% unmarked::modSel(.)
...
## Model selection
(final_mods_table <- unmarked::fitList(fits=listN(gdist_null_best, gdist_p_best, gdist_lambdafull, gdist_lambda1, gdist_full)) %>% unmarked::modSel(.))

gdist_best_mod <- gdist_p_best # If a more complex covariate model is within approx. 2AIC of best-fit model, use it.
...
# Also try with the complex model
...
(GDIST_MOD_NOTES <- list(
  ...
  gof = "NO GOOD--outside bounds for 5 of 6 best model and on edge for last. Same for best mod and complex mod.", 
  other = "Odd these models fit worse than single visit"))# Use this for any notes 
...
out_list <- list(
  ...
  umf_gds = umf_gds,
  final_mods = list(gdist_null_best, gdist_p_best, gdist_lambdafull, gdist_lambda1, gdist_full),
...
saveRDS(out_list, here::here("gdistsamp_mods", paste0("gdistsamp_", park, "_", spec, "_1visit_yearmod.RDS"))) # <<<<<<<<<<<<<<<<<<<<<<<<<<<




### SAVE ALL MODELS----
save(spec, park, max_bin_bound, combine_bins, single_visit, dat_gmix, dat_gdist, umf_gds, gdist_null_hn, file = here::here("gdistsamp_mods", paste0("gdistsamp_", save_prefix, ifelse(single_visit, "_1visit.RData", ".RData"))))




### Parametric bootstrap (may take ~10 minutes):
pb <- parboot(gdist_best_mod, nsim=80)
plot(pb) # Does model appear to fit the data well?
