...
# > I wonder if I can format each park's data for a species to add to the bbs data as a single "park transect"
# > I CAN'T FIGURE OUT HOW TO CHECK MODEL FIT BEYOND RHAT
# Look into the first difference model, which Link and Sauer 2020 say is better
# The models are overdispersed Poissons, and better with heavy tails (t-dist)

## ABOUT THE OUTPUTS:
# > Note that BBS is favoring the first-differenced model over the slopes one now (actually going toward the spatial)
# > Overdispersed Poisson generates a count-level random effect to account for extra-Poisson variance, following the approaches used in most of the official BBS analyses (note that NB is the default b/c uses less memory--so need to specify overdispersed Poisson).
...
# for (i in c("BITH", "GUIS", "JELA", "PAAL", "SAAN", "VICK")) {
#   park_bounds_list[[i]] <- FuncGetPark(i)
# }
# saveRDS(park_bounds_list, here::here("Data_out", "park_bounds_list.RDS"))
# 
# ### Pull in the GULN sf
