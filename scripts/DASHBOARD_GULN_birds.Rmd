...
FuncYrBoxPlotly <- function(dat, free_y) {
  ...
  add_annotations(
    ...
    font = list(size = 15)
      ...
      odd_dat <- plotdat %>% dplyr::filter(location_name %in% odd_sites) %>% as.data.frame(.)
    if(all(input$sel_highlight_points == TRUE, nrow(odd_dat) > 0)) { # highlight the "odd" points--will not totally line up though so will see them still in initial plot
...
FuncComboBoxPlotly <- function(dat = NULL, subplot_title = NULL) {
  ...
  odd_dat <- dat %>% dplyr::filter(location_name %in% odd_sites) %>% as.data.frame(.)
    if(all(input$sel_highlight_points == TRUE, nrow(odd_dat) > 0)) { # highlight the "odd" points--will not totally line up though so will see them still in initial plot
...
FuncHabPointPlotly <- function() {
  ...
  # subplot function
    func_subplot_habpoint <- function(plotdat, x_nam, y_nam) {
      ...
      odd_dat <- plotdat %>% dplyr::filter(location_name %in% odd_sites) %>% as.data.frame(.)
    if(all(input$sel_hab_highlight_points == TRUE, nrow(odd_dat) > 0)) { # highlight the "odd" points--will not totally line up though so will see them still in initial plot
   p_final <- add_markers(p,
                  data = odd_dat,
                  x = ~cov_x,
                y = ~cov_y,
...
renderUI({
  ...
  checkboxInput("sel_hab_highlight_points",
                label = strong("Highlight pre-defined 'odd' locations"),
                value = switch(is.null(input$highlight_points)+1, input$highlight_points, TRUE)
  )
})
```
...
output$table_specloc <- DT::renderDT({
 ...
 specloc_counts <- dat_list$df_full_obs_cov %>%
  ...
  dplyr::summarize(
   ...
   specloc_brks <- seq(0, max(specloc_counts$detections_per_survey, na.rm = TRUE), .1)
 ...
 table_dat <- dat_list$df_full_obs_cov %>%
   dplyr::filter(unit_code == input$sel_one_park) %>%
   dplyr::select(location_name, perc_forest_50, perc_forest_100, perc_forest_200, perc_opendev_50, perc_opendev_100, perc_opendev_200, prop_understory_cover_50, prop_understory_cover_100, prop_understory_cover_200, understory_cover_sd_50, understory_cover_sd_100, understory_cover_sd_100) %>%
 ...
 specloc_dt <- DT::datatable(
   ...
   class="compact stripe",
    ...
    options = list(
      ...
      formatStyle(columns = na.omit(str_extract(names(table_dat), "[:upper:]{4}")), target = "cell", backgroundColor = styleInterval(specloc_brks, specloc_cols)
 )
})
...
renderUI({
  ...
  checkboxInput("sel_highlight_points",
                label = strong("Highlight pre-defined 'odd' locations"),
                value = switch(is.null(input$highlight_points)+1, input$highlight_points, TRUE)
  )
})
