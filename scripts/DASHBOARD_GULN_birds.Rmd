...
dat_list <- list()
dat_list$df_finaldat <- readRDS(here::here("Data_out", "df_finaldat.RDS")) %>% 
  dplyr::mutate(
...
dat_list$df_locs <- readRDS(here::here("Data_out", "df_locs.RDS")) # site locations
...
dat_list$df_survey_conditions <- readRDS(here::here("Data_out", "df_survey_conditions.RDS")) %>% dplyr::filter(!is.na(time)) # event covariate data <<<<< INITIAL FORMATTING SHOULD HAVE TAKEN CARE OF REMOVING RECORDS WITH NO TIME ENTRY
df_full_obs <- readRDS(here::here("Data_out", "df_full_obs.RDS")) %>% dplyr::filter(!yr_visit %in% c("2014_2", "2016_2", "2018_2")) # <<<<<<<<<<<<<<<< BITH HAS ONE LOC (TC29) SURVEYED TWICE IN 2018, SO IT'S THE ONLY SITE WITH A YR_VISIT OF 2018_2 AND TC39 AS 2016_2. NEED TO CHECK ON THIS, BUT EXCLUDE IT FOR NOW.
...
dat_list$df_full_obs_cov <- df_full_obs %>%
  ...
  dplyr::left_join(dat_list$df_survey_conditions %>% dplyr::select(location_name, event_date, wk, hrs_since_rise, julian_prop, weather_wind, weather_wind_num, weather_temperature, weather_sky, weather_sky_revised_num, weather_noise, weather_noise_num), by = c("location_name", "event_date")) %>%
  dplyr::mutate(
    species_codename = paste0(common_name, " (", species_code, ")"),
    hover_text = paste0("<span style='font-size:12px; font-weight:bold;'>", unit_code, " ", yr_visit, "<br>", species_codename, "</span><br>Location: ", location_name, "<br>Habitat Type (200m): ", hab_type_200, "<br>Dominant Veg (200m): ", paste0(dom_veg_200, "(", dom_veg_prop_200, "%)"), "<br>Survey Date: ", event_date, "<br># of Individuals: ", sum_indiv))
...
rv$detect_by_loc <- dat_list$df_locs %>%
  dplyr::select(location_name, latitude, longitude, dom_veg_200, dom_veg_prop_200, hab_type_200) %>%
  ...
  dplyr::mutate(
    ...
    hover_text = paste0("<span style='font-size:12px; font-weight:bold;'>", location_name, " ", species_codename, "</span><br>Habitat: ", hab_type_200, "<br>Detections/Survey: ", detections_per_survey)
  )
...
rm(df_full_obs); rm(calc_recent_surv_round); rm(species_sparkbar); rm(temp_map_counts); 
...
FuncComboBoxPlotly <- function(dat = NULL, subplot_title = NULL) {
  ...
  p1 <- ggplotly(p, height = 250* (length(unique(rv$one_species_dat$unit_code)) + 3)) %>%
...
FuncCovPointPlotly <- function(dat) {
  # Get the data for continuous covariates in long format
  subdat <- dat %>%
    dplyr::select(unit_code, yr, location_name, species_codename, event_date, sum_indiv, wk, hrs_since_rise, julian_prop, weather_temperature) %>%
    tidyr::pivot_longer(cols = c("wk", "hrs_since_rise", "julian_prop", "weather_temperature"), names_to = "covariate_name", values_to = "covariate_value", values_drop_na = TRUE) %>%
    dplyr::mutate(
      hover_text = paste0("<span style='font-size:12px; font-weight:bold;'>", unit_code, " ", yr, "<br>", species_codename, "</span><br>", paste0(toupper(covariate_name), ": ", covariate_value), "<br># of Individuals: ", sum_indiv, "<br>Location: ", location_name, "<br>Survey Date: ", event_date))
  ...
  func_subplot_covpoint <- function(plotdat) {
    ...
    plotdat %>%
    dplyr::group_by(unit_code) %>%
    dplyr::mutate(fit = fitted(loess(sum_indiv ~ covariate_value))) %>%
    dplyr::ungroup() %>%
    plot_ly(height = total_page_ht) %>%
    add_markers(x = ~jitter(covariate_value), 
                y = ~jitter(sum_indiv),
                color = ~unit_code,
                ...
                hoverinfo = "text",
                showlegend = FALSE) %>%
      add_lines(x= ~covariate_value, y = ~fit, color = ~unit_code, showlegend = TRUE) %>%
    layout(
      yaxis = list(
        title = "Counts per survey"
      )
  ...
  add_annotations(
    text = switch(
      ...
      weather_temperature = "Air Temperature (C)"), # subplot title
    x = 0.5,
    y = 1.02,
    yref = "paper",
    xref = "paper",
...
saveRDS(dat_list, "temp_dat_list.RDS")
  ...
  rv$species_dist_dat <- temp_species_dist_dat %>%
    dplyr::left_join(dat_list$df_locs[c("location_name", "dom_veg_200", "dom_veg_prop_200", "hab_type_200")], by = "location_name") %>%
...
observeEvent(eventExpr = input$button_UpdateDetectFunctionPlots, {
  ...
  buffer_hab_list <- lapply(maps_list[grepl("_100m", names(maps_list))], FUN = function(x) {
    y<-x %>%
      as.data.frame() %>%
      dplyr::select(unit_code, location_name, buffer_m, veg_string) %>%
      tidyr::pivot_wider(names_from = buffer_m, values_from = veg_string, names_prefix = "buffer_") %>%
...
output$table_physiog <- DT::renderDT({
  ...
  tab_dat$dom_veg_200[tab_dat$dom_veg_200 == "Woodland and Forest"] <- "Woodlands and Forest"
  tab_dat %<>%
    dplyr::group_by(unit_code, hab_type_200, dom_veg_200) %>%
    dplyr::summarize(num_locs = n_distinct(location_name)) %>%
    tidyr::pivot_wider(names_from = "unit_code", values_from = "num_locs") %>%
    dplyr::rename(habitat = hab_type_200) %>%
    dplyr::arrange(habitat, dom_veg_200)
...
```{r}
output$table_one_park <- DT::renderDT({
...
DTOutput("table_one_park")
tags$style("#table_one_park{height:100vh;overflow-x:scroll;overflow-y:scroll;}")
```
...
renderUI({
  ...
  checkboxGroupInput("sel_species_parks",
                     ...
                     selected = switch(is.null(input$sel_species_parks)+1, input$sel_species_parks, FuncParkChoices(input$sel_common, input$sel_species))
                     )
  })
...
output$out_ComboBoxPlot <- renderPlotly({
  ...
  cov_vec <- c("hab_type_200", "weather_sky", "weather_wind", "weather_noise")
...
output$out_CovPointPlot <- renderPlotly({
  ...
  FuncCovPointPlotly(dat = rv$one_species_dat)
})
...
* <font size="2"> Each blue point is scaled to the detections/survey, averaged over all surveys conducted at that survey location. </font>
...
cat("line1224")   
  ...
  rv$species_map <- 
    ...
    addProviderTiles("Esri.WorldImagery", options = providerTileOptions(noWrap = TRUE), group="Satellite") %>%
    addProviderTiles("OpenStreetMap", group = "OpenStreetMap") %>%
    leaflet::addCircleMarkers(data = sp_map_dat, lng = ~longitude, lat = ~latitude, fill = TRUE, fillOpacity = 0.6, fillColor = "blue", weight = 1, color = "black", radius = ~point_size*sel_species_point_scale, label = ~point_text, popup = ~hover_text, group = "Avg Detections/Survey", options = pathOptions(pane = "point_size_counts")) %>%
...
output$table_species_survey <- renderReactable({
  shiny::req(!is.null(rv$one_species_dat))
  reactable::reactable(rv$one_species_dat %>% dplyr::select(species_codename, unit_code, location_name, yr, yr_visit, wk, event_date, hrs_since_rise, julian_prop, sum_indiv, hab_type_200, dom_veg_200, dom_veg_prop_200, weather_sky, weather_wind, weather_temperature, weather_noise),
                     defaultColDef = colDef(
                       align = "center"
                     ),
                     compact = TRUE,
...
hover_add <- maps_list[[paste0(sel_dist_map_park, "_100m")]] %>% 
  ...
  rv$linked_dist_map <-
    ...
    addMapPane("selected_points", zIndex = 420) %>%
      addProviderTiles("Esri.WorldImagery", options = providerTileOptions(noWrap = TRUE), group="Satellite") %>%
  addProviderTiles("OpenStreetMap", group = "OpenStreetMap") %>%
    leaflet::addCircleMarkers(data = linked_map_dat, lng = ~longitude, lat = ~latitude, fill = TRUE, fillOpacity = 0.6, fillColor = pal_map(linked_map_dat$dist_detect_function), weight = 1, color = "black", radius = ~point_size*sel_dist_point_scale, label = ~point_text, popup = ~hover_text, group = "Avg Detections/Survey", options = pathOptions(pane = "point_size_counts")) %>%
    ...
    leaflet::addCircleMarkers(data = linked_map_dat %>% highlight_key(~location_name, group = "Location"), lng = ~longitude, lat = ~latitude, fillOpacity = 0, stroke = TRUE, weight = 4, color = "blue", radius = ~point_size*sel_dist_point_scale, label = ~point_text, popup = ~hover_text, options = pathOptions(pane = "selected_points")) %>% # the linked data!
...
DT::renderDataTable({
  ...
  table_dat <- rv$linked_dist_obs_dat %>% dplyr::select(location_name, yr, event_date, event_comments, observer, time_bin, distance_bin_id, distance_bin, count, hab_type_200, weather_wind, weather_noise, point_notes)
  DT::datatable(
