...
FuncHabPointPlotly <- function() {
  ...
  # subplot function
    ...
    if(all(input$highlight_points == TRUE, nrow(odd_dat) > 0)) { # highlight the "odd" points--will not totally line up though so will see them still in initial plot
...
renderUI({
  ...
  checkboxInput("highlight_points",
                label = strong("Highlight pre-defined unusual locations"),
                selected = switch(is.null(input$highlight_points)+1, input$highlight_points, TRUE)
  )
})
