...
# > unmarked pcount() can do RE, but it's still "experimental" and the estimates are a bit different from ubms. Unfortunate, b/c pcount() can do ZIP and NB but stan_pcount can't. Also, pcount() is MUCH faster
...
## <<<<<< FOR GUIS & SAAN, DO SUBUNITS SEPARATELY
dat %<>% dplyr::filter(subunit == "MI")
...
site_covs <- dat_Nmix %>%
  dplyr::select(location_name, hab_type_100_comb, hab_type_200_comb, prop_understory_cover_50_sc, prop_understory_cover_100_sc, prop_understory_cover_200_sc, understory_cover_sd_50_sc, understory_cover_sd_100_sc, understory_cover_sd_200_sc) %>%
  dplyr::distinct() %>%
...
site_covs_st$yr_c_f <- rep(sort(unique(dat_Nmix$yr_c_f)), each = length(unique(na.omit(dat_Nmix$location_name)))) 
dim(site_covs_st)
head(site_covs_st)
...
### START THE YEARMOD MODELS ----
# UBMS can only do Poisson. Perhaps having habitat covariates in both detection and abundance equations can "account for" overdispersion??
# Site and year random effects, plus year fixed effect
# Each ubms model takes ~30+ minutes!
# Warning about number of sites discarded because of missing data--these are the site-year combinations with NA for both visits

## Start with pcount to find best-fit model faster

# Try different models with pcount, to see if Poisson is adequate. Unmarked N-mixture models give questionable results when the distribution is NB, so if there is no Poisson model with adequate model fit, I am not sure I would use N-mixture. 
m0_pois <- pcount( # NULL model
  ~1 
  ~yr_c_f + (1|location_name),
  data = umf, 
  chains=4, 
  iter=4000, 
  thin = 2,
...
FuncFitList(listN(m0_pois, m0_zip, m0_nb)) # Check if Poisson model is best-fit or close enough to best fit that can use stan_pcount. pcount's use of random effects is unstable and not totally trustworthy.

# Add covariates to the POISSON null model. Not clear which habitat covariates should be in detection model vs abundance model vs both, so try the different combinations and then compare models
# For starters, try the covariates that were in the best-supported glmmTMB year model. In general, habitat type should definitely be included in abundance model and possibly in detection model; understory cover variables should likely be included in both.
mod_tmb <- readRDS(here::here("tmb_mods_yearmod", paste0("tmb_", park, "_", spec, "_100m_yearmod.RDS")))
mod_tmb$call

m1 <- update(m0_pois, formula = 
               ~prop_understory_cover_200_sc # DETECTION covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
             ~yr_c_f + (1|location_name) # ABUNDANCE covariates--could be habitat variables. 
             )
m2 <- update(m0_pois, formula = 
               ~1 # DETECTION covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
             ~prop_understory_cover_200_sc + yr_c_f + (1|location_name) # ABUNDANCE covariates--could be habitat variables. If subunit, multiply subunit by year.
)

m3 <- update(m0_pois, formula = 
               ~prop_understory_cover_200_sc # DETECTION covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
             ~prop_understory_cover_200_sc + yr_c_f + (1|location_name) # ABUNDANCE covariates--could be habitat variables. 
)


m4 <- update(m0_pois, formula = 
               ~prop_understory_cover_200_sc  # DETECTION covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
             ~understory_cover_sd_100_sc + yr_c_f + (1|location_name) # ABUNDANCE covariates--could be habitat variables. 
)
m5 <- update(m0_pois, formula = 
               ~1 # DETECTION covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
             ~understory_cover_sd_100_sc + prop_understory_cover_200_sc + yr_c_f + (1|location_name) # ABUNDANCE covariates--could be habitat variables. If subunit, multiply subunit by year.
)

m6 <- update(m0_pois, formula = 
               ~prop_understory_cover_200_sc # DETECTION covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
             ~understory_cover_sd_100_sc + prop_understory_cover_200_sc + yr_c_f + (1|location_name) # ABUNDANCE covariates--could be habitat variables. 
)
...
### Compare models
mod_list <- listN(m0_pois, m1, m2, m3, m4, m5, m6)
FuncFitList(mod_list)
best_mod <- m4 # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

# Save the modlist with ALL models >>>>>> ADD SUBUNIT IF NEEDED
saveRDS(mod_list, here::here("stan_pcount_mods", paste0("pcount_", park, "_", spec, "_100m_yearmod_modlist.RDS")))

### IF THE STAN_PCOUNT POISSON MODEL JUST DOESN'T WORK, AND A ZIP MODEL IS BETTER, CAN TRY THAT WITH PCOUNT...
summary(best_mod)

## Check model fit
best_mod_nm <- update(best_mod, method='Nelder-Mead')
(best_mod_pb <- parboot(best_mod_nm, fitstats2, nsim = 30, report = 1, ncores = 4))

AICcmodavg::Nmix.gof.test(best_mod, nsim = 60) 

### RUN THE BEST MODEL WITH STAN_PCOUNT ----
# NOTE: Can only do Poisson, not ZIP or NB, with stan_pcount, currently. If NB is the best fit, MIGHT be worth trying observation-level random effects, but I don't if that would have the same "unrealistic predictions" issue as NB N-mixture models??

best_mod@call

# This one uses habitat variables in both the detection and abundance equations
m_stan <- stan_pcount( 
  ~prop_understory_cover_200_sc # DETECTION
  ~understory_cover_sd_100_sc + 
    yr_c_f + (1 | location_name), # ABUNDANCE covariates--could be habitat variables. If subunit, multiply subunit by year.
  data = umf, 
  chains=4, 
  iter=4000, 
  thin = 2,
...
# make sure estimates seem reasonable
ubms::plot_marginal(mod, "state", "yr_c_f")
ubms::plot_marginal(mod, "det")
...
plot_residuals(mod, submodel = "state", covariate = "yr_c_f")
plot_residuals(mod, submodel = "state", covariate = "understory_cover_sd_50_sc")
plot_residuals(mod, submodel = "det", covariate = "prop_understory_cover_50_sc")
...
# Check the pareto k of best mod
...
# If all okay, SAVE THE MODEL
saveRDS(mod, here::here("stan_pcount_mods", paste0("pcount_", park, "_", spec, "_100m_yearmod.RDS")))
...
# # This seems to give similar but not identical results as plot_effects()
# mod@call$formula
...
# 
# mod <- stan_pcount( 
#   # ~researcher + first_yr + weather_wind_comb + prop_understory_cover_200 +  understory_cover_sd_200 + julian_prop_c + hrs_since_rise_c + I(hrs_since_rise_c^2) # detection covariates first--could be researcher, weather_wind_comb, julian_prop_c, hrs_since_rise_c, or any habitat variable
#   # ~yr_sc + (1|location_name) + (1|yr_f) + prop_understory_cover_200 +  understory_cover_sd_200, # abundance covariates--could be habitat variables. If subunit, multiply subunit by year.
#   data = umf, 
#   chains=4, 
#   iter=4000, 
#   thin = 2,
