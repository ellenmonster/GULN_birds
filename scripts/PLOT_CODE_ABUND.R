...
### DONE:
# > NOCA: JELA, BITH, PAAL, VICK
# > WEVI: JELA, BITH
### User entry----
park <- "JELA"  # "BITH", "JELA", "PAAL", "VICK", 
spec <- "CACH" 
...
(temp_tmb_pred <- ggemmeans(mod_tmb, terms=c("yr_c"), type = "fixed"))
...
nd = data.frame(yr_sc = sort(unique(mod_pcount@data@siteCovs$yr_sc)), 
                ...
                understory_cover_sd_100 = mean(mod_pcount@data@siteCovs$understory_cover_sd_100),
...
# geom_line(data = pcount_pred, aes(x=yr, y=trend_predicted), color = "red", linetype = "dotted") +
  ...
  labs(title = park, x = "Year", y = "Index of abundance (# of birds/point count)", size = "# of point counts") +
...
saveRDS(p, here::here("RESULTS", "abund_plots", paste0("plot_abund_", park, "_", spec, ".RDS")))
...
(p <- ggplot() + 
 ...
 geom_errorbar( # glmmTMB YEAR RESULTS AS POINTS W/ERROR BARS
   ...
   labs(title = "Trends in relative abundance of Northern cardinal at PAAL", x = "Year", y = "Index of abundance (# of birds/point count)", size = "# of point counts") +
   theme_bw()
   # geom_vline(xintercept = 2018.5, linewidth = 2, color = "lightgray") 
# geom_text() + annotate("text", label = "Every location surveyed 2X/yr since 2019", x = 2018.6, hjust = 0, y = 6.5, size = 2.75) +
# geom_segment(aes(x=2018.5, y=6.1, xend=2022, yend=6.1), arrow = arrow(length=unit(.2, 'cm'))) +
# geom_text() + annotate("text", label = "Half of sites surveyed each year before 2018", x = 2017.6, hjust = 1, y = 6.5, size = 2.75) +
# geom_segment(aes(x=2017.5, y=6.1, xend=2014, yend=6.1), arrow = arrow(length=unit(.2, 'cm')))
)
