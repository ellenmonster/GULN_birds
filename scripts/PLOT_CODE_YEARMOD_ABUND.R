...
FuncTMB <- function(mod, mod_type, num_visits) {
  pred <- ggeffects::predict_response(model = mod, margin = "mean_reference", terms=c("yr_c_f"), type = ifelse(tmb_zi, "zero_inflated", "fixed")) %>%  # Note that to get emmeans, 'margin = "marginalmeans"' but then can't do zero-inflation with that
    as.data.frame() %>%
    janitor::clean_names() %>%
    dplyr::mutate(
...
FuncTMBSubunit <- function(mod, mod_type, num_visits, subunit) {
  pred <- ggeffects::predict_response(model = mod, margin = "mean_reference", terms=c("subunit", "yr_c_f"), type = ifelse(tmb_zi, "zero_inflated", "fixed")) %>%  # Note that to get emmeans, 'margin = "marginalmeans"' but then can't do zero-inflation with that
    as.data.frame() %>%
    janitor::clean_names() %>%
    dplyr::filter(x == subunit) %>%
    dplyr::mutate(
      yr_c = as.numeric(as.character(group)),
      inf_ci = is.infinite(conf_high),
      n_visits = num_visits,
      mod = mod_type) %>%
    dplyr::select(
...
mod_stanpcount_yearmod <- readRDS(here::here("stan_pcount_mods", paste0("pcount_", park, "_", spec, "_100m_yearmod.RDS")))  # <<<<<<<< FOR SUBUNITS, REPLACE PARK NAME WITH SUBUNIT NAME, E.G., "GUIS-FL"
...
# # ## glmmTMB-predicted values for SUBUNIT
# pred_tmb1 <- FuncTMBSubunit(mod = mod_tmb_1visit_yearmod, mod_type = "GLMM (index of abundance)", num_visits = 1, subunit = "MI")
# pred_tmb2 <- FuncTMBSubunit(mod = mod_tmb_yearmod, mod_type = "GLMM (index of abundance)", num_visits = 2, subunit = "MI")

### Stan_pcount-predicted values----

pred_stan <- plot_marginal(mod_stanpcount_yearmod, submodel = "state", covariate = "yr_c_f")$data %>%
   dplyr::mutate(
...
# dataframe for plot
pdat <- rbind.data.frame(pred_tmb1, pred_tmb2, pred_stan) # <<<<< REMOVE AS NEEDED

### FULL PLOT ----
# >>>>> REMOVE SUBTITLE AS NEEDED
(p <- ggplot(data = pdat, aes(group = n_visits)) + 
  ...
  geom_point(
   ...
   scale_x_continuous(breaks=min(pdat$yr, na.rm = TRUE):max(pdat$yr, na.rm = T)) +
  labs(title = paste(toupper(park), toupper(spec), "within 100m radius"), subtitle = "(NOTE: Y-AXIS SCALES DIFFER for top & bottom plots)", x = "Year", y = "Birds per point count (error bars are 95%CI)") +
 theme_bw() +
